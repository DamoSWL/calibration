...
def data_loader(dataset, data_path=datadir, normalization="AugNormAdj", porting_to_torch=True, task_type = "full"):
    if dataset in ["amazon_electronics_computers", "amazon_electronics_photo", "ms_academic_cs", "ms_academic_phy"]:
        (adj,
         ...
         degree,
         learning_type,
         test_mask_id) = load_npz_data(dataset, SEED, task_type) 
        train_adj = adj
        ...
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_test, degree, learning_type, test_mask_id
        ...
        (adj,
         ...
         degree,
         learning_type,
         test_mask_id) = load_citation(dataset, normalization, porting_to_torch, data_path, task_type)
        train_adj = adj
        train_features = features
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_test, degree, learning_type, test_mask_id
...
def load_citation(dataset_str="citeseer", normalization="AugNormAdj", porting_to_torch=True, data_path=datadir, task_type="full"):
    ...
    if porting_to_torch:
        ...
        test_mask_id = torch.BoolTensor(test_mask_id)
    learning_type = "transductive"
    return adj, features, labels, idx_train, idx_val, idx_test, degree, learning_type, test_mask_id
...
def data_loader_OOD(dataset, data_path=datadir, normalization="NoNorm", porting_to_torch=True, task_type = "full"):
    if dataset in ["cora", "citeseer", "pubmed"]:
        ...
        (adj,
         ...
         idx_test_id,
         test_mask_id,
         test_mask_ood) = load_ogb_data_ood_train(dataset) 
        train_adj = adj
        train_features = features
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_test, degree, learning_type, idx_test_ood, idx_test_id, test_mask_id, test_mask_ood
        ...
        (adj,
         ...
         idx_test_id,
         test_mask_id,
         test_mask_ood) = load_heterophily_ood(dataset) 
        train_adj = adj
        train_features = features
        return adj, train_adj, features, train_features, labels, idx_train, idx_val, idx_test, degree, learning_type, idx_test_ood, idx_test_id, test_mask_id, test_mask_ood
...
def load_heterophily_ood(dataset_str):
    data = WikipediaNetwork('data', dataset_str).data

    adj = torch_geometric.utils.to_scipy_sparse_matrix(data.edge_index)
    features = np.array(data.node_stores[0]['x'])
    labels = np.array(data.node_stores[0]['y'])
    adj = adj.maximum(adj.T)
    labels = labels.flatten()

    idx_train = np.array(list(range(2277))[:500])
    idx_val = np.array(list(range(2277))[500:1000])
    idx_test = np.array(list(range(2277))[1200:])

    random_state = np.random.RandomState(42)
    idx_train = filter_idx(idx_train,labels,20,random_state)
    idx_val = filter_idx(idx_val,labels,30,random_state)
...
def load_citation_ood(dataset_str="citeseer", normalization="NoNorm", porting_to_torch=True, data_path=datadir, task_type="full"):
    """
    Load Citation Networks Datasets.
    """
    names = ['x', 'y', 'tx', 'ty', 'allx', 'ally', 'graph']
...
def load_ogb_data_ood_train(dataset_str):  
    dataset = PygNodePropPredDataset(name=dataset_str,transform=T.ToSparseTensor())
    data = dataset[0]
    features = data.x.numpy()

    data.adj_t = data.adj_t.to_symmetric()
    # adj = sp.coo_matrix(data.adj_t.to_torch_sparse_coo_tensor().to_dense().numpy())
    ...
    random_state = np.random.RandomState(42)
    idx_train = filter_idx(idx_train.numpy(),labels,200,random_state)
    idx_val = filter_idx(idx_val.numpy(),labels,220,random_state)
...
def preprocess_citation(adj, features, normalization="NoNorm"):
